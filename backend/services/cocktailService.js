...
const cocktailService = {
   ...
   async updateCocktail(id, data) {
      ...
      const newCocktail = { name, base, image, description, ingredient, tags, recipes, abv, sweet, bitter, sour };
      const targetKeys = Object.keys(newCocktail);
      const isSame = targetKeys.every((key) => foundCocktail[key] === newCocktail[key]) && targetKeys.length === Object.keys(foundCocktail).length;
