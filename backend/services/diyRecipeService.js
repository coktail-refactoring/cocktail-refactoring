...
const diyRecipeService = {
  ...
  async updateDiyRecipe(userId, id, newImageNames, recipeImageNames, data) {
    ...
    const newDiyRecipe = { name, base, image, description, ingredient, tags, recipes, abv, sweet, bitter, sour, };
    const targetKeys = Object.keys(newDiyRecipe);
    const isSame = targetKeys.every((key) => foundDiyRecipe[key] === newDiyRecipe[key]) && targetKeys.length === Object.keys(foundDiyRecipe).length;
