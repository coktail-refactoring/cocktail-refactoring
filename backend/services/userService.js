...
const userService = {
   ...
   async updateUser(userId, { email, nickname }) {
      const foundUser = await User.findOne({ _id: userId, deletedAt: null }).lean();
      if (!foundUser) throw new NotFoundError("사용자 정보 없음");

      const newUser = { email, nickname };
      const targetKeys = Object.keys(newUser);
      const isSame = targetKeys.every((key) => foundUser[key] === newUser[key]) && targetKeys.length === Object.keys(foundUser).length;
